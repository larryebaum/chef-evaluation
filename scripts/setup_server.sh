...
# check for the existing users and create if needed
for user in admin cicdsvc
do
  sudo chef-server-ctl user-show | grep "${user}"
  if [[ $? != 0 ]]; then
    sudo chef-server-ctl user-create $user first last $user@test.com "${user}pwd" --filename ${GUEST_WKDIR}/.chef/${user}.pem
  fi
done
...
if [[ $? != 0 ]]; then
  ...
  sudo chef-server-ctl org-user-add a2 cicdsvc --admin
fi
